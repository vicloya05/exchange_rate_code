...
#Total RMSE for 10 days ahead
rmse_arima_da_drift <- sqrt(mean((actual_prices_next_days - arima_111_da_drift)^2))
rmse_arima_da <- sqrt(mean((actual_prices_next_days - arima_111_da)^2))
rmse_knn_da <- sqrt(mean((actual_prices_next_days - knn_da)^2))

#Calculate the RMSE for each day ahead
rmse_arima_da_drift <- numeric(10)
rmse_arima_da <- numeric(10)
rmse_knn_da <- numeric(10)
# Loop through i values from 1 to 10
for (i in 1:10) {
  rmse_arima_da_drift[i] <- sqrt(mean((actual_prices_next_days[1:i] - arima_111_da_drift[1:i])^2))
  rmse_arima_da[i] <- sqrt(mean((actual_prices_next_days[1:i] - arima_111_da[1:i])^2))
  rmse_knn_da[i] <- sqrt(mean((actual_prices_next_days[1:i] - knn_da[1:i])^2))
}

all_rmse_horizons <- cbind(rmse_arima_da_drift,rmse_arima_da,rmse_knn_da)
print(all_rmse_horizons)
